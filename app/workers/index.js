...
const init = () => {
     ...
     */
    const addTradePairsForAnalysisQueue = new Queue('add-trade-pairs-for-analysis', 1, 'redis://redis:6379');
     ...
     */
    const analyzeTradePairQueue = new Queue('analyze-trade-pair', 3, 'redis://redis:6379');
     ...
     */
    const openNewDealQueue = new Queue('open-new-deal', 3, 'redis://redis:6379');
     ...
     */
    const checkOpenDealsQueue = new Queue('check-open-deals', 1, 'redis://redis:6379');
    checkOpenDealsQueue.process(__dirname + '/check-open-deals.js');
    /**
     * Adds stop loss order for given deal (only for uptrend deals)
     */
    const addStopLossOrderQueue = new Queue('add-stop-loss-order', 3, 'redis://redis:6379');
     ...
     */
    const addTakeProfitOrderQueue = new Queue('add-take-profit-order', 3, 'redis://redis:6379');
    addTakeProfitOrderQueue.process(__dirname + '/add-take-profit-order.js');
    /**
     * Checks the status of open (NEW) orders
     */
    const checkOpenOrdersQueue = new Queue('check-open-orders', 1, 'redis://redis:6379');
    checkOpenOrdersQueue.process(__dirname + '/check-open-orders.js');
    /**
     * Replaces stop loss order with new one (higher price)
     */
    const replaceStopLossOrderQueue = new Queue('replace-stop-loss-order', 3, 'redis://redis:6379');
     ...
     */
    const replaceTakeProfitOrderQueue = new Queue('replace-take-profit-order', 3, 'redis://redis:6379');
     ...
     */
    const addSymbolsForAnalysisQueue = new Queue('add-symbols-for-analysis', 1, 'redis://redis:6379');
     ...
     */
    const prepareSymbolInfoQueue = new Queue('prepare-symbol-info', 3, 'redis://redis:6379');
     ...
     */
    const updateExchangeInfoQueue = new Queue('update-exchange-info', 1, 'redis://redis:6379');
    updateExchangeInfoQueue.process(__dirname + '/update-exchange-info.js');
    /**
     * System garbage collector
     */
    const maintenanceQueue = new Queue('maintenance', 1, 'redis://redis:6379');
